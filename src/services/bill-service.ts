...
export const previewBillsForBuilding = async (
	...
	try {
		...
		throw new Error(extractErrorMessage(error, 'Không thể xem trước hóa đơn') );
	}
};
...
export const getBills = async (
	...
	try {
		...
		const response = await apiClient.get<PaginatedBillResponse>(endpoint);
...
export const getBillById = async (
	...
	try {
		const response = await apiClient.get<{ data: Bill }>(`/api/bills/${id}`);
		return normalizeEntityResponse<Bill>(response.data);
	} catch (error) {
		throw new Error(extractErrorMessage(error, 'Không thể tải chi tiết hóa đơn'));
	}
};
...
export const updateBill = async (
	...
	try {
		const response = await apiClient.patch<{ data: Bill }>(
			`/api/bills/${id}`,
		    data,
		);
		return normalizeEntityResponse<Bill>(response.data);
	} catch (error) {
		throw new Error(extractErrorMessage(error, 'Không thể cập nhật hóa đơn'));
	}
};
...
export const deleteBill = async (
	...
	try {
		...
		throw new Error(extractErrorMessage(error, 'Không thể xóa hóa đơn'));
	}
};
...
export const markBillAsPaid = async (
	...
	try {
		...
		throw new Error(extractErrorMessage(error, 'Không thể đánh dấu đã thanh toán'));
	}
};
...
export const generateMonthlyBillsForBuilding = async (
	...
	token?: string,
): Promise<GenerateMonthlyBillsResponse> => {
	try {
		const response = await apiClient.post<GenerateMonthlyBillsResponse>(
			'/api/bills/generate-monthly-bills-for-building',
			data,
		);
		return response.data;
	} catch (error) {
		throw new Error(extractErrorMessage(error, 'Không thể tạo hóa đơn hàng tháng cho toà nhà'));
	}
};
...
export const updateBillWithMeterData = async (
	...
	try {
		...
		throw new Error(extractErrorMessage(error, 'Không thể cập nhật hóa đơn với dữ liệu đồng hồ'));
	}
};
