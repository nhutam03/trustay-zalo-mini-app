...
import type { TableCell, TableColumn } from '@/interfaces/ai';
...
export const AITablePreview: React.FC<AITablePreviewProps> = ({
	columns,
	rows,
	previewLimit,
	onOpenFull,
		...
		if (col.type === 'image' && typeof cell === 'string') {
			return (
				<img
					src={cell}
					alt={col.label}
					className={`${compact ? 'w-10 h-10' : 'w-20 h-20'} object-cover rounded border`}
				/>
			);
		}
		...
		if (rowPath && (col.key === 'title' || col.key === 'id')) {
			...
			return isExternal ? (
				<a
					...
					target="_blank"
					rel="noopener noreferrer"
					className="text-blue-700 underline block max-w-[200px] truncate"
				>
				...
				<button
					...
					className="text-blue-700 underline block max-w-[200px] truncate text-left"
				>
					{truncate(String(cell))}
				</button>
			);
		...
		<div className="mt-3 overflow-auto">
						...
						<div className="overflow-auto max-h-[70vh]">
							<table className="min-w-full text-sm border bg-white">
								<thead className="bg-gray-50">
									<tr>
										{columns.map((col) => (
											<th key={col.key} className="text-left px-3 py-2 border-b">
												{col.label}
											</th>
										))}
									</tr>
			...
			>
				<thead className="bg-gray-50">
					<tr>
						{columns.map((col) => (
							<th key={col.key} className="text-left px-2 py-1 border-b">
								{col.label}
							</th>
						))}
					</tr>
