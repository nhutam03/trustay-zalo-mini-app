...
interface AIPostRoomModalProps {
	visible: boolean;
	onClose: () => void;
...
export const AIPostRoomModal: React.FC<AIPostRoomModalProps> = ({ visible, onClose, onRoomCreated }) => {
	...
	const handleResponse = (response: RoomPublishResponse) => {
		...
		switch (status) {
			...
			case RoomPublishingStatus.CREATED:
				...
				if (data.payload?.roomId && onRoomCreated) {
					setTimeout(() => {
						onRoomCreated(data.payload!.roomId!);
						onClose();
					}, 2000);
				}
				break;
	...
	return (
		<Modal visible={visible} onClose={onClose} title="Đăng phòng nhanh với AI">
			...
			{dialogState === 'clarification' && (
				<>
					...
					<div className="max-h-64 overflow-y-auto space-y-3 mb-4 bg-gray-50 rounded p-3">
					...
					<>
						...
						<Box className="space-y-3 mb-4 max-h-96 overflow-y-auto">
							<Text bold className="text-gray-800 mb-2">📋 Thông tin sẽ được tạo:</Text>
							
							{/* Building Info */}
							{publishPlan.payload.plan.shouldCreateBuilding && publishPlan.payload.plan.buildingPayload && (
								<Box className="bg-white border border-gray-200 rounded-lg p-3 space-y-2">
									<div className="flex items-center gap-2 mb-2">
										<Text bold size="small" className="text-blue-600">🏢 Tòa nhà mới</Text>
									</div>
									<div className="space-y-1 text-sm">
										<div className="flex">
											<span className="text-gray-600 w-24">Tên:</span>
											<span className="text-gray-800 font-medium">{publishPlan.payload.plan.buildingPayload.name}</span>
										</div>
										<div className="flex">
											<span className="text-gray-600 w-24">Địa chỉ:</span>
											<span className="text-gray-800">{publishPlan.payload.plan.buildingPayload.addressLine1}</span>
										</div>
										<div className="flex">
											<span className="text-gray-600 w-24">Khu vực:</span>
											<span className="text-gray-800">Ward {publishPlan.payload.plan.buildingPayload.wardId}, District {publishPlan.payload.plan.buildingPayload.districtId}</span>
										</div>
									</div>
								</Box>
							)}
							...
							{publishPlan.payload.plan.roomPayload && (
								<Box className="bg-white border border-gray-200 rounded-lg p-3 space-y-2">
									<div className="flex items-center gap-2 mb-2">
										<Text bold size="small" className="text-green-600">🚪 Thông tin phòng</Text>
									</div>
									<div className="space-y-1 text-sm">
										<div className="flex">
											<span className="text-gray-600 w-32">Tên phòng:</span>
											<span className="text-gray-800 font-medium">{publishPlan.payload.plan.roomPayload.name}</span>
										</div>
										{publishPlan.payload.plan.roomPayload.areaSqm && (
											<div className="flex">
												<span className="text-gray-600 w-32">Diện tích:</span>
												<span className="text-gray-800">{publishPlan.payload.plan.roomPayload.areaSqm} m²</span>
											</div>
										)}
										{publishPlan.payload.plan.roomPayload.maxOccupancy && (
											<div className="flex">
												<span className="text-gray-600 w-32">Số người tối đa:</span>
												<span className="text-gray-800">{publishPlan.payload.plan.roomPayload.maxOccupancy} người</span>
											</div>
										)}
											...
											<>
												<div className="flex">
													...
													<span className="font-semibold text-red-600">
														{publishPlan.payload.plan.roomPayload.pricing.basePriceMonthly?.toLocaleString('vi-VN')} đ/tháng
													</span>
												</div>
												<div className="flex">
													<span className="text-gray-600 w-32">Tiền cọc:</span>
													<span className="text-gray-800">
														{publishPlan.payload.plan.roomPayload.pricing.depositAmount?.toLocaleString('vi-VN')} đ
													</span>
												</div>
											</>
										)}
										{publishPlan.payload.plan.roomPayload.description && (
											<div className="mt-2 pt-2 border-t border-gray-100">
												...
												<div 
													...
													dangerouslySetInnerHTML={{ __html: publishPlan.payload.plan.roomPayload.description }}
												/>
											</div>
										)}
										{publishPlan.payload.plan.roomPayload.amenities && publishPlan.payload.plan.roomPayload.amenities.length > 0 && (
											<div className="mt-2 pt-2 border-t border-gray-100">
												<span className="text-gray-600 block mb-1">Tiện nghi:</span>
												<div className="text-gray-800 text-xs">
													{publishPlan.payload.plan.roomPayload.amenities.length} tiện nghi được chọn
												</div>
											</div>
										)}
										{publishPlan.payload.plan.roomPayload.images?.images && publishPlan.payload.plan.roomPayload.images.images.length > 0 && (
											<div className="mt-2 pt-2 border-t border-gray-100">
												<span className="text-gray-600 block mb-1">Hình ảnh:</span>
												<div className="flex gap-2 flex-wrap">
													{publishPlan.payload.plan.roomPayload.images.images.slice(0, 3).map((img, idx) => (
														<div key={idx} className="w-16 h-16 bg-gray-100 rounded border overflow-hidden">
															<img 
						...
						<div className="flex gap-2">
							<Button onClick={onClose} disabled={isCreating} variant="secondary" fullWidth>
								Hủy
							</Button>
				...
				{dialogState === 'created' && (
					<Box className="flex flex-col items-center justify-center p-8 space-y-6">
						...
						<Box className="w-full space-y-3">
							<Button
								...
								onClick={() => {
									onClose();
									if (onRoomCreated && publishPlan?.payload?.roomId) {
										onRoomCreated(publishPlan.payload.roomId);
									}
								}}
							>
								Xem phòng đã tạo
